...
- **Support .flame and Other File Types**
  Add import/export support for JWildfire's .flame XML format and other fractal file types (e.g., Apophysis .apo, ChaosPro .chaos). Implement parsers/serializers to convert between formats, enabling compatibility with existing flame libraries and sharing scenes across tools.
...
- **Improve Path Animation**
  Fix path animation to reduce jittery and overly fast movement. Currently, path position changes every frame; implement smoother interpolation (e.g., easing, frame rate independence, or keyframe blending) to make animations more fluid and controllable.
